...
def create_batches(data, labels, batch_size, create_bit_vector=False):
    N = data.shape[0]
    if N % batch_size != 0:
        print( "Warning in create_batches(): Batch size {0} does not " \
...
batch_size = 100
train_data, train_labels = create_batches(train_set[0], train_set[1],
                                              batch_size,
                                              create_bit_vector=True)
valid_data, valid_labels = create_batches(valid_set[0], valid_set[1],
                                              batch_size,
                                              create_bit_vector=True)

mlp = MLP(layer_dimensions=[784, 100, 100, 10], batch_size=batch_size)
